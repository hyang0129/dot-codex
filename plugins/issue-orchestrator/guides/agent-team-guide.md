...
Use this guide when `/fix-issue` needs real Codex sub-agent orchestration.

## Core principle

The main command run is the orchestrator. It should not pretend that "agent teams" are a separate product feature. In Codex, the team is built explicitly with:

- `spawn_agent`
- `wait_agent`
- `send_input`

Use those tools deliberately and keep the critical path local when possible.

## Codex role mapping

| Workflow role | Codex shape | Model | Notes |
|---|---|---|---|
| Orchestrator | main command run | inherited | owns branching, git, and handoffs |
...
- run compile/typecheck/lint/test commands as appropriate,
...
The convenience promise of `/fix-issue` is that it continues inline whenever the flow is safe to continue. Do not reduce it to "now go type another command" unless a human decision is genuinely required.
